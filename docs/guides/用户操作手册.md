...
- **进入主页面**：点击左侧活动栏的 **OpenSkills** 图标后，会自动打开 Web 主页面；也可通过命令 `OpenSkills: 打开主页面 (Open Panel)` 打开。
...
### 3.7 API 服务未成功运行（含「API 又没自动启动」）

- **现象**：扩展或 Web 提示 API 不可用、健康检查失败，或「API 没有成功运行」「API 又没自动启动」。
- **自动启动前提**：扩展在激活时会尝试自动启动 API（需同时满足：设置中 **OpenSkills › Auto Start Servers** 为开启、以**文件夹**形式打开工作区且存在 `packages/api`、且 API 已编译出 `packages/api/dist/index.js`）。若任一条不满足，扩展会跳过或失败并弹出提示；可查看 **OpenSkills 输出通道** 中的 `[Servers]` 日志确认原因。
- **常见原因与处理**：

| 原因 | 说明 | 处理 |
|------|------|------|
| **自动启动被关闭** | 设置中「Auto Start Servers」为关闭 | 在 Cursor 设置中搜索 OpenSkills，开启 **Auto Start Servers**；或手动在终端执行 `npm run dev:api`（项目根目录） |
| **扩展内嵌 API 未编译** | 扩展用 `node dist/index.js` 启动 API，若未构建则无 `dist/index.js` | 在项目根目录执行 `npm run build`，或进入 `packages/api` 执行 `npm run build`，再重试扩展内嵌服务 |
| **端口不一致** | 扩展默认连 3847；若用 `npm run dev:api` 且未设端口，API 会在 3000 | 根目录 `npm run dev` 已统一为 API 3847、Web 3848；单独启动 API 时请设 `PORT=3847`（如 `cross-env PORT=3847 npm run dev:api`）或保持扩展配置的 Api Port 与 API 实际端口一致 |
| **端口被占用** | 3847 或 3000 已被其它程序占用 | 关闭占用端口的程序，或修改扩展配置「OpenSkills: Api Port」与 API 的 `PORT` 环境变量为其它空闲端口 |
| **工作区路径错误** | API 需从工作区根找到 `.openskills` | 用「在终端启动」时请在**项目根目录**执行 `npm run dev` 或 `npm run dev:api`；扩展内嵌启动时会传 `WORKSPACE_ROOT`，一般无需改 |

- 建议先运行 **「OpenSkills: Diagnose」** 或 **「OpenSkills: Open Diagnose Panel」** 查看 API 基地址与健康检查结果；输出通道中的 `[API]`、`[Servers]` 日志可帮助确认是否编译、端口与启动错误。
...
- **快速参考**：`QUICK_REFERENCE.md`（启动、修复命令、CLI 安装、唤醒）
- **如何唤醒**：`如何唤醒skills-admin.md`（安装 CLI、验证唤醒）
